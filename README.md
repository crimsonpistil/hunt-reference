...
**Where substitution applies.** Arkime and Kibana fields, which are the ones you paste into a search bar. Suricata rules keep their variables. Substitution reaches the rendered query, every Copy button (per-block, copy-all, and the header quick-copy), and the My Hunts TXT / CSV / CMS exports, so a package handed to TheHive is not still full of `$MPNET`.
