...
const DATA = [
    ...
    rows: [
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] HTTP beacon - periodic GET to same URI with low jitter, characteristic of Cobalt Strike, Sliver, Mythic, Havoc",
      ...
      {
        sub: "T1071.001 - Web Protocols",
        indicator: "[OFF-NET TRIPWIRE] HTTP POST beacon - outbound POST with small body, characteristic of beacon check-in / task response",
      ...
      {
        sub: "T1071.001 - Web Protocols",
        indicator: "[OFF-NET TRIPWIRE] HTTPS beacon - periodic TLS sessions to same destination with low jitter and small data volumes",
        arkime: "ip.src == $MPNET\n&& protocols == tls\n&& port.dst == 443\n&& packets.src < 30\n&& packets.dst < 30\n&& databytes.src < 10000\n&& session.length < 5",
      ...
      {
        sub: "T1071.001 - Web Protocols",
        indicator: "[OFF-NET TRIPWIRE] HTTP User-Agent anomaly - non-browser UA on browser-like traffic, missing UA, or known-malicious UA",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Outbound FTP from non-file-server host - interactive FTP session for C2 or staging",
        arkime: "ip.src == $MPNET\n&& ip.src != $FILE_SERVERS\n&& protocols == ftp\n&& port.dst == 21\n&& ip.dst != $ALLOWED_DEFAULTS\n&& session.length > 30",
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Internal host querying legitimate service for IP/port calculation seed - DNS calculation precursor",
    ...
    rows: [
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Internal host fetching paste from Pastebin / Ghostbin / Hastebin / Rentry - dead drop C2 endpoint resolution",
      ...
      {
        sub: "T1102.001 - Dead Drop Resolver",
        indicator: "[OFF-NET TRIPWIRE] GitHub Gist or raw repository content fetch from non-developer host - dead drop via GitHub",
        ...
        notes: "For process attribution, pivot to Kibana with Sysmon Event 3 filtered by the destination IP/port from this session - read winlog.event_data.Image for the culprit process. [AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. GitHub Gists and raw repository content are commonly abused as dead drops because GitHub is essentially impossible to block in modern enterprises. The adversary publishes a gist containing the C2 endpoint configuration, and the implant fetches the raw URL. Detection requires distinguishing legitimate developer activity (git clone, IDE syncing, package managers fetching from GitHub) from anomalous fetches by other processes. EDR process correlation is essential. Build per-host baselines: developer workstations have heavy legitimate GitHub traffic; finance / HR / production server access to gist.githubusercontent.com is highly anomalous. Filter on URL patterns too - implants typically fetch /username/gist-id/raw paths, not browsing patterns.",
        apt: [
      ...
      {
        sub: "T1102.001 - Dead Drop Resolver",
        indicator: "[OFF-NET TRIPWIRE] Twitter/X profile or post fetch from non-browser process - social media dead drop",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Discord webhook POST from non-Discord-client process - Discord-based C2 channel",
      ...
      {
        sub: "T1102.002 - Bidirectional Communication",
        indicator: "[OFF-NET TRIPWIRE] Telegram Bot API POST from non-Telegram-client process - Telegram-based C2 channel",
      ...
      {
        sub: "T1102.002 - Bidirectional Communication",
        indicator: "[OFF-NET TRIPWIRE] Slack webhook POST from non-Slack-client process - corporate Slack abuse for C2",
        ...
        notes: "For process attribution, pivot to Kibana with Sysmon Event 3 filtered by the destination IP/port from this session - read winlog.event_data.Image for the culprit process. [AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. Slack incoming webhooks (https://hooks.slack.com/services/T{team}/B{bot}/{token}) are abused for C2 and exfil similarly to Discord webhooks - no authentication required beyond knowing the URL, accepts arbitrary text and file content. Slack API endpoints (chat.postMessage, files.upload) provide bidirectional C2 with bot token authentication. Detection challenge: Slack is heavily used in enterprises, so blocking is not feasible. Focus on process correlation: legitimate Slack traffic comes from the Slack desktop app, browser tabs, or sanctioned integrations (Jira, GitHub, PagerDuty, Datadog). Anything else POSTing to Slack endpoints is suspicious. Maintain inventory of approved Slack integrations.",
        apt: [
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Cloud storage POST/PUT from non-storage-client process - exfil to Dropbox / OneDrive / Google Drive / Mega",
      ...
      {
        sub: "T1102.003 - One-Way Communication",
        indicator: "[OFF-NET TRIPWIRE] Google Docs / Drive API POST from non-Google-client process - document-based C2 or exfil",
    ...
    rows: [
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Outbound TLS with known-malicious JA3 / JA4 client fingerprint - implant TLS handshake match",
      ...
      {
        sub: "T1573 - JA3/JA4 Client Fingerprint",
        indicator: "[OFF-NET TRIPWIRE] JA4 fingerprint anomaly - client fingerprint never seen on this host or VLAN before",
      ...
      {
        sub: "T1573 - JA3/JA4 Client Fingerprint",
        indicator: "[OFF-NET TRIPWIRE] JA4 fingerprint inconsistent with claimed User-Agent - TLS client lying about browser identity",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Self-signed TLS certificate on connection to external host - implant team server",
        ...
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. Modern public-internet TLS uses certificates signed by trusted CAs (Let's Encrypt, DigiCert, Sectigo, GoDaddy, etc). A self-signed certificate on an external HTTPS connection is a strong anomaly - operators sometimes deploy self-signed certs on team servers to avoid the work of getting a real cert, or for short-lived infrastructure. Detection: issuer == subject in the certificate chain. Internal services (development environments, IoT devices, internal CAs) legitimately use self-signed certs - exclude internal traffic. The remaining external self-signed traffic is essentially always either misconfigured legitimate infrastructure (rare) or implant infrastructure (common).",
        apt: [
      ...
      {
        sub: "T1573 - Certificate Anomalies",
        indicator: "[OFF-NET TRIPWIRE] Recently-issued Let's Encrypt certificate on connection to first-seen destination",
        arkime: "ip.src == $MPNET\n&& protocols == tls\n&& cert.issuer.cn == \"*Let's Encrypt*\"\n&& port.dst == 443\n&& ip.dst != $ALLOWED_DEFAULTS\n// First-seen destination filtering requires SPIView per-host history or\n// SIEM correlation - Arkime expression cannot natively express 'this host's\n// first connection to this destination'. Use cert.notBefore in SPIView to\n// sort and filter for recently-issued certs (last 7 days).",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] High-entropy payload in cleartext protocol - non-TLS encrypted C2 channel",
    ...
    rows: [
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] ICMP echo with anomalously large payload - tunneled data in ping packets",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Outbound connection to known offensive tooling default ports - netcat, msfvenom listener defaults",
      ...
      {
        sub: "T1095 - Raw TCP / UDP Shells",
        indicator: "[OFF-NET TRIPWIRE] Raw TCP on common web ports without HTTP/TLS - protocol mismatch indicating bind shell",
        ...
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. Adversaries often use ports 80, 443, 8080, 8443 for raw TCP shells because these ports are universally allowed outbound - but the traffic is raw command/response, not HTTP or TLS. Zeek's Dynamic Protocol Detection (DPD) identifies the actual protocol regardless of port. Raw TCP on a web port = strong indicator. Suricata's app-layer-protocol negation works similarly. The detection catches bind shells, custom binary protocols, encrypted-but-not-TLS payloads, and raw netcat-style channels. False positives: some custom internal applications use raw TCP on web ports (this should be on internal IPs only); some IoT devices use unusual protocols on standard ports.",
        apt: [
      ...
      {
        sub: "T1095 - Raw TCP / UDP Shells",
        indicator: "[OFF-NET TRIPWIRE] Raw UDP outbound on non-standard port - UDP-based C2 channel",
        ...
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. Outbound UDP traffic from internal hosts is mostly DNS (53), NTP (123), some VPN protocols (500/4500/51820), syslog (514), DHCP (67/68), and a small set of others. Custom UDP C2 channels stand out - high packet rate from internal hosts to external destinations on non-standard UDP ports. Build $ALLOWED_UDP allowlist for sanctioned UDP traffic (your DNS resolvers, NTP servers, sanctioned VPN endpoints). After exclusions, sustained UDP traffic to external destinations is anomalous. UDP-based C2 is less common than TCP because UDP doesn't provide reliability - but some sophisticated implants use it for stealth (no connection tracking).",
        apt: [
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] GRE / SCTP / AH / ESP outbound from non-router host - L3 protocol abuse",
      ...
      {
        sub: "T1095 - L3/L4 Protocol Abuse",
        indicator: "[OFF-NET TRIPWIRE] Anomalous TCP flag combinations - covert channels in TCP header fields",
        kibana: "source.ip: $MPNET\nAND tcp.flags: (\n  \"FIN+URG+PSH\"\n  OR \"NULL\"\n  OR \"FIN\"\n)",
        suricata: "alert tcp $HOME_NET any\n  -> $EXTERNAL_NET any\n  (msg:\"TA0011 T1095 Anomalous\n    TCP flag combination scan\n    or covert channel\";\n  flags:FPU,12;\n  classtype:bad-unknown;\n  sid:9109508; rev:1;)",
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. TCP flag combinations like FIN+URG+PSH (Christmas tree scan), NULL (no flags set), or lone FIN are not used by legitimate TCP stacks - they're either scan attempts or covert channel signaling. Some advanced implants use unusual flag combinations or sequence number patterns for low-bandwidth covert C2 (each connection attempt encodes 1-2 bits of data). The detection catches both reconnaissance scans and the rare custom implant that uses TCP header fields as a covert channel. False positives: stack fingerprinting tools (nmap, masscan with custom options) - these should originate from sanctioned scan sources only. Outbound from a workstation = essentially always anomalous.",
        apt: [
    ...
    rows: [
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] netsh PortProxy relay - host listening on an arbitrary TCP port and forwarding traffic across segments (Volt Typhoon signature LOTL pivot)",
        arkime: "ip.dst == $MPNET\n&& ip.src == $MPNET\n&& port.dst > 1024\n&& port.dst != $KNOWN_SERVICE_PORTS\n&& tcp.flags.syn == 1\n&& tcp.flags.ack == 0",
        kibana: "destination.ip: $MPNET\nAND source.ip: $MPNET\nAND destination.port > 1024\nAND NOT destination.port: $KNOWN_SERVICE_PORTS\nAND network.transport: tcp\n/* pivot on hosts that BOTH receive and re-originate */",
        suricata: "alert tcp $HOME_NET any\n  -> $HOME_NET any\n  (msg:\"TA0011 T1090.001 netsh\n    portproxy relay - host bridging\n    two internal conversations\n    (Volt Typhoon LOTL pivot)\";\n  flow:established,to_server;\n  flowbits:set,portproxy.in;\n  threshold:type both,\n    track by_dst,\n    count 3, seconds 600;\n  classtype:trojan-activity;\n  sid:9109010; rev:1;)",
        notes: "[AIR-GAP TRIPWIRE] netsh interface portproxy add v4tov4 is one of Volt Typhoon's most documented network behaviors (CISA AA24-038A). They turn a compromised edge device, server, or domain controller into a quiet relay using only the built-in Windows netsh command - no malware, no extra tooling, which is why host AV misses it. The network signal is the giveaway: a host that should not be a server suddenly accepts an inbound TCP connection on an odd high port and, in the same window, originates an outbound connection toward another segment or external infrastructure - it is bridging two conversations. Hunt method: build per-host listener baselines and alert on first-seen listening ports, then correlate inbound-to-port-X with a paired outbound from the same host. The portproxy config also persists in the registry under HKLM\\\\SYSTEM\\\\CurrentControlSet\\\\Services\\\\PortProxy\\\\v4tov4\\\\tcp and shows in netsh interface portproxy show all on the host - pull both during triage. Volt Typhoon has used this on PRTG servers and edge appliances specifically to proxy traffic toward critical-infrastructure OT boundaries. Replace $KNOWN_SERVICE_PORTS with your real service-port allowlist so legitimate app servers do not drown the signal.",
        apt: [
          { cls: "apt-cn", name: "Volt Typhoon", note: "Signature behavior: netsh interface portproxy on compromised edge devices, PRTG servers, and DCs to relay traffic toward US critical-infrastructure networks while staying living-off-the-land (CISA AA24-038A)." },
          { cls: "apt-cn", name: "APT41", note: "Uses netsh and built-in port forwarding for internal pivoting in long-dwell intrusions." },
          { cls: "apt-mul", name: "Multi", note: "netsh portproxy is a common LOTL pivot once an actor has admin on a Windows host - low-tooling and persistent." }
        ],
        cite: "MITRE ATT&CK T1090.001, T1599, CISA AA24-038A (Volt Typhoon)"
      },
      {
        sub: "T1090.001 - Internal Proxy",
        ...
        apt: [
          { cls: "apt-cn", name: "Volt Typhoon", note: "Turns compromised edge devices and servers into internal relays (often via netsh portproxy) to route traffic toward critical-infrastructure targets." },
      ...
      {
        sub: "T1090.002 - External Proxy",
        indicator: "[OFF-NET TRIPWIRE] HTTP CONNECT method outbound - open proxy abuse for tunneling",
        arkime: "ip.src == $MPNET\n&& protocols == http\n&& http.method == CONNECT\n&& ip.dst != $MPNET_PROXIES\n&& ip.dst != $ALLOWED_DEFAULTS",
        kibana: "source.ip: $MPNET\nAND http.request.method: CONNECT\nAND NOT destination.ip: $MPNET_PROXIES\nAND NOT destination.ip: $ALLOWED_DEFAULTS",
        suricata: "alert http $HOME_NET any\n  -> $EXTERNAL_NET any\n  (msg:\"TA0011 T1090.002 HTTP\n    CONNECT method to external\n    open proxy abuse\";\n  flow:established,to_server;\n  content:\"CONNECT \"; depth:8;\n  classtype:trojan-activity;\n  sid:9109004; rev:1;)",
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. HTTP CONNECT is used by web proxies to establish tunnels (typically HTTPS through a forward proxy). Legitimate use: clients configured with $MPNET_PROXIES tunneling out via your sanctioned forward proxy. Anomalous use: a workstation issuing CONNECT to an external IP that isn't your proxy - indicates either misconfigured client or implant using an external open proxy as a relay. Open proxies (free, anonymous proxy lists) are heavily abused for adversary anonymization. The CONNECT method is rarely seen in normal HTTP traffic; when it appears outbound to non-corporate-proxy destinations it's a strong signal. Zeek http.log captures the method explicitly.",
        apt: [
      ...
      {
        sub: "T1090.003 - Multi-hop Proxy",
        indicator: "[OFF-NET TRIPWIRE] obfs4 / meek bridge traffic - Tor obfuscation pluggable transport",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] SNI / Host header mismatch on HTTPS connection - canonical domain fronting signature",
      ...
      {
        sub: "T1090.004 - Domain Fronting",
        indicator: "[OFF-NET TRIPWIRE] Outbound TLS to high-volume CDN domain on first contact - possible domain fronting endpoint",
        ...
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. CDN-hosted endpoints carry massive volumes of legitimate traffic - most large websites use CloudFront, Akamai, Fastly, or Azure CDN. The challenge is distinguishing legitimate CDN-fronted services from C2 infrastructure on the same CDN. Per-host first-contact analysis helps: a workstation establishing a sustained connection to a never-before-seen CDN endpoint subdomain is more suspicious than connecting to a popular SaaS service's CDN. Build per-host CDN endpoint history. Pair with destination certificate examination - domain-fronted C2 often uses cloudfront.net wildcard certs while legitimate services use brand-specific certs. Cloud Functions URLs (cloudfunctions.net, *.run.app) are increasingly abused - these are dynamically allocated and trivial for adversaries to register.",
        apt: [
  ...
  },
  {
    ...
    rows: [
      {
        sub: "T1572 - GRE Tunnel via Device Config",
        indicator: "[OFF-NET TRIPWIRE] New GRE tunnel (protocol 47) between network devices or to external infrastructure - config-based covert routing (Salt Typhoon)",
        arkime: "ip.protocol == 47\n&& (ip.src == $NETWORK_DEVICES || ip.dst == $NETWORK_DEVICES)\n&& ip.dst != $SANCTIONED_GRE_PEERS",
        kibana: "network.iana_number: 47\nAND (source.ip: $NETWORK_DEVICES\n  OR destination.ip: $NETWORK_DEVICES)\nAND NOT destination.ip: $SANCTIONED_GRE_PEERS",
        suricata: "alert ip $NETWORK_DEVICES any\n  -> any any\n  (msg:\"TA0011 T1572 GRE tunnel\n    from network device - config\n    based covert routing\n    (Salt Typhoon)\";\n  ip_proto:47;\n  threshold:type both,\n    track by_src,\n    count 1, seconds 3600;\n  classtype:trojan-activity;\n  sid:9157201; rev:1;)",
        notes: "Salt Typhoon modifies the running-config of compromised switches and routers to stand up Generic Routing Encapsulation (GRE, IP protocol 47) tunnels, then routes collection and exfiltration traffic through them - blending into normal carrier/router-to-router traffic and side-stepping inspection that only watches TCP/UDP. GRE between routers and VPN concentrators is legitimate, so the signal is a NEW or unexpected GRE peering: a device tunneling to an endpoint that is not in your sanctioned peer list, or a device that never spoke GRE suddenly originating it. Hunt method: baseline your GRE peerings ($SANCTIONED_GRE_PEERS) and alert on first-seen tunnels, especially those terminating outside your AS or toward hosting/VPS ranges. Pair with config-integrity monitoring on the device (diff running-config against a known-good baseline for tunnel interface stanzas) and with the loopback/ACL changes Salt Typhoon makes to source traffic from trusted addresses. Because the tunnel lives in device config rather than on a host, EDR will not see it - this is a network-and-config hunt.",
        apt: [
          { cls: "apt-cn", name: "Salt Typhoon", note: "Modified device configurations to create and use GRE tunnels for routing collection/exfil traffic through compromised carrier infrastructure." },
          { cls: "apt-cn", name: "Volt Typhoon", note: "Edge-device-centric routing manipulation aligns with their living-off-the-land infrastructure tradecraft." },
          { cls: "apt-mul", name: "Multi", note: "GRE and other L3 tunnels on compromised network gear are a stealthy, EDR-invisible routing channel for infrastructure-focused actors." }
        ],
        cite: "MITRE ATT&CK T1572, T1599, Salt Typhoon reporting"
      },
      {
      ...
      {
        sub: "T1572 - SSH Tunneling",
        indicator: "[OFF-NET TRIPWIRE] SSH session with sustained bidirectional data - long-lived tunnel rather than interactive shell",
      ...
      {
        sub: "T1572 - HTTPS Tunneling / WebSocket",
        indicator: "[OFF-NET TRIPWIRE] WebSocket upgrade on HTTP - long-lived bidirectional channel inside HTTP/HTTPS",
      ...
      {
        sub: "T1572 - HTTPS Tunneling / WebSocket",
        indicator: "[OFF-NET TRIPWIRE] HTTPS connection with sustained bidirectional flow - tunneled session inside TLS",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] DNS-over-HTTPS (DoH) endpoint - bypass of on-network DNS inspection",
    ...
    rows: [
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] certutil.exe outbound HTTP - Windows certificate utility used as downloader",
        arkime: "ip.src == $MPNET\n&& protocols == http\n&& http.user-agent == [\"*CertUtil*\", \"*Microsoft-CryptoAPI*\"]\n&& ip.dst != $ALLOWED_UPDATE_SOURCES\n&& ip.dst != $ALLOWED_DEFAULTS",
        kibana: "source.ip: $MPNET\nAND user_agent.original: (\n  *CertUtil*\n  OR *Microsoft-CryptoAPI*\n)\nAND NOT destination.ip: $ALLOWED_UPDATE_SOURCES",
      ...
      {
        sub: "T1105 - LOLBin User-Agent Fetches",
        indicator: "[OFF-NET TRIPWIRE] bitsadmin.exe / BITS service download - background transfer service abuse",
        ...
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. Background Intelligent Transfer Service (BITS) is the Windows service that handles Windows Update, WSUS, and similar background downloads. Its User-Agent is 'Microsoft BITS/{version}' and is highly distinctive. Adversaries abuse BITS via 'bitsadmin /transfer' or PowerShell's Start-BitsTransfer to download payloads with administrative tooling that's signed and trusted. BITS to non-Microsoft-Update destinations is anomalous. Build $ALLOWED_UPDATE_SOURCES allowlist (Microsoft published ranges, your WSUS server). Anything else from BITS is suspicious. The technique is documented across many APTs because BITS downloads run in the background, persist across reboots, and don't require an interactive user session. EDR correlation with bitsadmin.exe process or PowerShell Start-BitsTransfer cmdlet provides definitive identification.",
        apt: [
      ...
      {
        sub: "T1105 - LOLBin User-Agent Fetches",
        indicator: "[OFF-NET TRIPWIRE] PowerShell HTTP download - Invoke-WebRequest / DownloadString outbound fetch",
        ...
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. PowerShell's Invoke-WebRequest, Invoke-RestMethod, and (New-Object Net.WebClient).DownloadString send a default User-Agent containing 'WindowsPowerShell' or 'Mozilla/5.0 (Windows NT 10.0; ...; WindowsPowerShell/...)'. PowerShell HTTP downloads are rare in normal user activity (sometimes legitimate for IT automation scripts, package managers like Chocolatey) but very common in adversary tooling. Detection: any PowerShell HTTP UA from a host that doesn't legitimately use PowerShell for HTTP. Build per-host allowlists for sanctioned PowerShell HTTP patterns. The Empire and PowerSploit frameworks use PowerShell HTTP downloads extensively; modern Cobalt Strike powerpick and execute-assembly use in-memory loading that doesn't always cross the network. Pair with PowerShell logging (Event ID 4104) for definitive correlation.",
        apt: [
      ...
      {
        sub: "T1105 - LOLBin User-Agent Fetches",
        indicator: "[OFF-NET TRIPWIRE] WinHTTP / wininet / curl / wget User-Agent - non-browser HTTP libraries downloading executables",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Executable download from low-reputation TLD or newly registered domain",
      ...
      {
        sub: "T1105 - Payload Source Anomalies",
        indicator: "[OFF-NET TRIPWIRE] Executable downloaded from IP address rather than domain - direct-IP fetch bypassing DNS reputation",
    ...
    rows: [
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] TLS handshake on non-standard port - HTTPS-style traffic on port other than 443/8443",
        ...
        notes: "[AIR-GAP TRIPWIRE] This indicator detects outbound traffic from MPNET to off-MPNET infrastructure. In a properly air-gapped environment this query should never produce hits; any hit indicates a likely air-gap violation: bridged USB tether, rogue cellular modem, vendor/maintenance laptop bridging networks, supply-chain implant calling home, or misconfigured perimeter device. Treat as priority-1 escalation; do not dismiss as false positive without thorough investigation. TLS legitimately runs on a small set of ports: 443 (HTTPS), 8443 (alternative HTTPS), 4443 (alternative), 465/587 (SMTPS submission), 636 (LDAPS), 853 (DNS-over-TLS), 989/990 (FTPS), 993 (IMAPS), 995 (POP3S), 8883 (MQTTS). TLS on other ports is anomalous - often C2 servers configured on non-standard ports for stealth (4433, 5443, 8081, etc). The Suricata content '|16 03|' matches the TLS record header (version 03 = TLSv1.x). Some legitimate corner cases exist: development environments, custom enterprise applications using TLS on bespoke ports - these should be in $ALLOWED_INTERNAL or have well-defined destination IPs to exclude. Outbound to internet on TLS-non-standard port is rarely legitimate.",
        apt: [
  ...
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] ConnectWise ScreenConnect / NinjaRMM / Atera / Splashtop - RMM platform connections",
      ...
      },
      {
        ...
        indicator: "[OFF-NET TRIPWIRE] Tunneling tool infrastructure - ngrok, Cloudflare Tunnel, FRP, Chisel endpoints",
