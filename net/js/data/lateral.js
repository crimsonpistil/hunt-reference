...
const DATA = [
  ...
  },
  {
    ...
    rows: [
      {
        sub: "T1021.004 - Device SSH from Loopback (ACL Bypass)",
        indicator: "[OFF-NET TRIPWIRE] Device-to-device SSH sourced from a switch/router loopback or infrastructure address - ACL bypass pivot (Salt Typhoon)",
        ...
        suricata: "alert tcp $NETWORK_DEVICES any\n  -> $NETWORK_DEVICES 22\n  (msg:\"TA0008 T1021.004 SSH between\n    network devices from loopback\n    source - ACL bypass pivot\n    (Salt Typhoon)\";\n  flow:established,to_server;\n  content:\"SSH-\"; depth:4;\n  classtype:trojan-activity;\n  sid:9100404; rev:1;)",
        notes: "Salt Typhoon modifies the loopback interface address on a compromised switch and then uses that loopback as the SOURCE of SSH connections to additional devices - because management ACLs often trust traffic from infrastructure/loopback ranges, this bypasses the access controls that would block an arbitrary host. The result is device-to-device SSH that looks like it came from trusted infrastructure. Hunt method: SSH (TCP/22) where BOTH ends are network devices and the source is not one of your sanctioned admin/jump sources ($SSH_ADMINS). Even more telling: SSH sourced from a loopback address that does not match the device's known/baselined loopback, or a loopback that recently changed. Pair with config-integrity monitoring for interface Loopback stanza changes and ACL edits (T1686), and with new authorized_keys appearing on the target devices (Salt Typhoon adds SSH authorized_keys under root). This is infrastructure lateral movement - it never touches a host OS, so it is invisible to endpoint tooling and must be caught on the wire and in device config.",
        apt: [
          { cls: "apt-cn", name: "Salt Typhoon", note: "Modified the loopback address on compromised switches and used it as the SSH source to reach additional devices, bypassing management ACLs; also planted SSH authorized_keys for persistence." },
          { cls: "apt-cn", name: "Volt Typhoon", note: "Infrastructure-to-infrastructure pivoting via trusted addresses fits their edge-device living-off-the-land approach." },
          { cls: "apt-mul", name: "Multi", note: "Sourcing traffic from trusted infrastructure/loopback ranges to defeat ACLs is a general technique once an actor controls network gear." }
        ],
        cite: "MITRE ATT&CK T1021.004, T1098.004, T1686, Salt Typhoon reporting"
      },
      {
